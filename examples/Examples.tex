...
$$\begin{cases}
  \gamma_t = \Sigma(x) \gamma_x + C_0 \gamma\\
  \alpha(0,t) = q\beta(0,t) \\
  \beta(1,t) = 0
\end{cases}$$

where $\gamma = \left[\alpha, \beta\right]^T$ and $C_0 = \begin{pmatrix} c_1(x) & 0 \\ 0 & c_4(x) \end{pmatrix}$
...
\begin{align*}
  \Sigma(x)K_x(x,y) + K_y(x,y)\Sigma(y) &= K(x,y)\left[C(y)-\Sigma'(y)-C_0(x)\right] \\
...
\begin{align}
  ...
  \epsilon_1(x)K^{uv}_x -\epsilon_2(y)K^{uv}_y &= \left[\epsilon_2'(y)-c_4(y)+c_4(x)\right]K^{uv} -c_2(y)K^{uu} \\
  \epsilon_2(x)K^{vu}_x - \varepsilon_1(y)K^{vu}_y &= \left[c_1(y) + \epsilon_1'(y)-c_1(x)\right]K^{vu} + c_3(y)K^{vv} \\
...
\begin{align*}
  ...
  &\text{\hspace{1em}}
  + \sum_{k=0}^{i-1-j}K^{uv}_{(k+j)j}c^4_{i-1-j-k}
...
\begin{align*}
  ...
  &\text{\hspace{1em}}
  - \sum_{k=0}^{i-1-j}K^{vu}_{(k+j)j}c^1_{i-1-j-k}
