...
\begin{itemize}
    ...
    \item \code{getindex} -- The \code{getindex} method is another way of accessing a series coefficients. Calling for instance \code{ps[1,2]} will return a \code{ScalarSeriesSymbol} that can then be called using the \code{getindex} method an additionnal time which will return a \code{SeriesCoefficient}. Calling for instance \code{ps[1,2][3]} will return the corresponding \code{SeriesCoefficient}. Using then \code{getNum} on this \code{SeriesCoefficient} will return its value if it has already been computed and throw an error otherwise. Thus, \code{getNum(ps[1,2][3])} is the same as \code{ps.coefficients[1,2][3]}. 
